			...
			if (isPlayerTurn) {
				...
				for (int i = 0; i < Direction.values().length; i++) {
					...
					if (boardOfChildNode.canMove(Direction.values()[i])) {
						...
						childNode.setParentMove(i);
						this.setHeuristicScore(childNode, boardOfChildNode);
						temp.add(childNode);
					}
				}
				...
				for (int i = 0; i < temp.size(); i++) {
					Board boardOfChildNode = (Board) board.clone();
					Node childNode = new Node();
					int j = temp.get(i).getParentMove();
					if (boardOfChildNode.canMove(Direction.values()[j])) {
						boardOfChildNode.move(Direction.values()[j]);
						childNode = alphaBeta(boardOfChildNode, depth - 1, false, alpha, beta);
						alpha = Math.max(alpha, childNode.getValue());
						childNode.setValueDirOfChild(Direction.values()[j]);
						childNode.setActualScore(boardOfChildNode.getActualScore());
				...
				for (int emptyCellPos = 0; emptyCellPos < board.getNumberOfEmptyCells(); emptyCellPos++) {
					...
					for (int valueOfCell : newvalue) {
						...
						Node child = new Node();
						this.setHeuristicScore(child, boardOfChild);
						child.setValueOfCell(valueOfCell);
						child.setCol(col);
						child.setRow(row);
						temp.add(child);
					}
				...
				for (int i = temp.size()-1; i >= 0; i--) {
					Board boardOfChild = (Board) board.clone();
					Node newNode = temp.get(i);
					boardOfChild.setValueToCell(newNode.getValueOfCell(), newNode.getRow(), newNode.getCol());
					Node child = new Node();
...
//					System.out.println("i = "+i+" value = "+node.getChildren().get(i).getValue());
//				}
//				node.setValue(alpha);
//			} else {
//				int[] newvalue = { 2, 4 };
