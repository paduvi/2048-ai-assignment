...
public class Board implements Cloneable {
	...
	public final Random randomGernerator = new Random();
	...
	private boolean addRandomCell() throws Exception{
		...
		if(listSize > 0){
			int randomCellId = emptyCells.get(this.randomGernerator.nextInt(listSize));
			int randomValue = (this.randomGernerator.nextDouble() < 0.9) ? 2 : 4;
	...
	public int move(Direction direction) {
		...
		case NONE:
			return 0; 
		default:
			System.out.println("Unknown Direction!"); return 0;
		}
	}
	...
	public boolean isTerminated() throws CloneNotSupportedException {
		...
		if (hasWon()){
			...
			if(this.getNumberOfEmptyCells() == 0){
				// The only case that the game may be terminated is that there is no empty cells.
	...
	/**
	 * Player wins the game iff there is a cell whose value is greater than TARGET_POINT
	 * If player's actual score is not greater than MINIMUM_WIN_SCORE, he (definitely) has NOT won yet.
	 */
	...
	private int getClusteringScoreByHorizontal(){
		...
		for(int row = 0; row < this.getSize(); row++){
			for(int col = 0; col < this.getSize() - 1; col++){
				clusteringScoreByHorizontal += 
						Math.abs(this.cells[row][col] - this.cells[row][col + 1]);
			}
		}
	...
	private int getClusteringScoreByVertical(){
		...
		for(int col = 0; col < this.getSize(); col++){
			for(int row = 0; row < this.getSize() - 1; row++){
				clusteringScoreByVertical += 
						Math.abs(this.cells[row][col] - this.cells[row + 1][col]);
			}
		}
